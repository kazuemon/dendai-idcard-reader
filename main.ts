...
const pcsc = pcsclite();
const CONTROL_CODE = parseInt(process.env.CONTROL_CODE ?? '0x003136b0');
...
pcsc.on('reader', async (reader) => {
  ...
  const conProtocol = await new Promise<number>((r, d) =>
    reader.connect(
      { share_mode: reader.SCARD_SHARE_DIRECT, protocol: 0 },
      (e, protocol) => (e ? d(e) : r(protocol)),
    ),
  );

  console.log('Direct connected: ' + reader.name);

  const startSesRes = await new Promise<Buffer>((r, d) =>
    reader.control(
      Buffer.from([0xff, 0xc2, 0x00, 0x00, 0x02, 0x81, 0x00, 0x00]),
      CONTROL_CODE,
      7,
      (e, res) => (e ? d(e) : r(res)),
    ),
  ...
  const switchPrtRes = await new Promise<Buffer>((r, d) =>
    reader.control(
      Buffer.from([0xff, 0xc2, 0x00, 0x02, 0x04, 0x8f, 0x02, 0x03, 0x00, 0x00]),
      CONTROL_CODE,
      7,
      (e, res) => (e ? d(e) : r(res)),
    ),
  ...
  const pollingTimerId = setInterval(async () => {
    const pollingRes = await new Promise<Buffer>((r, d) =>
      reader.control(
        Buffer.from([
    ...
    if (pollingRes.length >= 4 && pollingRes[3] === 0x90) {
      ...
      const readBlockRes = await new Promise<Buffer>((r, d) =>
        reader.control(
          Buffer.from([
            0xff,
            0xc2,
